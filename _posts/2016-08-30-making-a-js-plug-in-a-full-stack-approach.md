...
Imagine that we allow our clients to modify the positioning and styles dynamically of the widget (that way it will be integrated smoothly).
...
### 2.2. Identifying Client Web Sites
...
Trying to solve this problem, we considered using `iframe`s, as you may know, `iframe`s are not so popular when you're trying to look like something secure.
